// ignore: unused_import
import 'package:intl/intl.dart' as intl;
import 'app_localizations.dart';

// ignore_for_file: type=lint

/// The translations for Ukrainian (`uk`).
class AppLocalizationsUk extends AppLocalizations {
  AppLocalizationsUk([String locale = 'uk']) : super(locale);

  @override
  String get hello => 'Привіт';

  @override
  String get deleteAll => 'Видалити все';

  @override
  String get sureToDeleteAllQuestions =>
      'Ви впевнені, що хочете видалити всі запитання?';

  @override
  String get welcome => 'Ласкаво просимо до застосунку';

  @override
  String get edit_sets => 'Редагувати картки';

  @override
  String get settings => 'Налаштування';

  @override
  String get refresh_questions => 'Оновити запитання';

  @override
  String get upload_questions => 'Завантажити запитання';

  @override
  String get adults => 'Дорослі';

  @override
  String get kids => 'Діти';

  @override
  String get languages => 'Мови';

  @override
  String get primary_language => 'Основна мова';

  @override
  String get secondary_language => 'Додаткова мова';

  @override
  String get pick_category => 'Обери категорію';

  @override
  String get category_picker_header_subtitle => 'Обери теми для наступної гри';

  @override
  String category_picker_selected_count(int count) {
    return 'Обрано: $count';
  }

  @override
  String get category_picker_select_all => 'Обрати все';

  @override
  String get category_picker_clear => 'Очистити';

  @override
  String get category_picker_card_subtitle_adults => 'Роздуми та зв\'язок';

  @override
  String get category_picker_card_subtitle_kids => 'Гра та уява';

  @override
  String get set_up_categories => 'Налаштуй категорії';

  @override
  String get categories_settings_info_title => 'Категорії';

  @override
  String get categories_settings_info_description =>
      'Обери категорії, які хочеш бачити в грі. Використай «Усі», щоб швидко вибрати або зняти вибір з усіх.';

  @override
  String get categories_settings_all => 'Усі';

  @override
  String get remove_ad => 'Прибрати рекламу назавжди';

  @override
  String get restore_purchase => 'Відновити покупку';

  @override
  String get pick_to_edit => 'Обери для редагування';

  @override
  String get preferences => 'Уподобання';

  @override
  String get show_animation => 'Показувати анімацію';

  @override
  String get font_size => 'Розмір шрифту';

  @override
  String get show_animation_when_swiping_cards =>
      'Показувати анімацію при перегортанні карток';

  @override
  String get explore => 'Обрати';

  @override
  String get start_game => 'Розпочати гру';

  @override
  String get choose_at_least_one_category_to_start_game =>
      'Оберіть принаймні одну категорію, щоб розпочати гру';

  @override
  String get category => 'Категорії';

  @override
  String get info => 'Інформація';

  @override
  String get share => 'Поділитися';

  @override
  String get set_up_the_category_list => 'Налаштування категорій';

  @override
  String get conversations => 'Спілкування';

  @override
  String get questions => 'Питання';

  @override
  String get that_matter => 'що має значення';

  @override
  String get nothing_here_yet => 'Тут поки що нічого немає.';

  @override
  String get try_to_choose_different_category =>
      'Спробуйте вибрати іншу категорію';

  @override
  String get time_to_choose_the_categories_for_the_game =>
      'Час обрати категорії.';

  @override
  String get failed_to_load_questions =>
      'Не вдалося завантажити питання. Будь ласка, спробуйте пізніше.';

  @override
  String get please_fill_in_question_text =>
      'Будь ласка, заповніть текст питання';

  @override
  String get question_added => 'Питання додано!';

  @override
  String get failed_to_add_question => 'Не вдалося додати питання:';

  @override
  String get new_question => 'Нове питання';

  @override
  String get submit => 'Надіслати';

  @override
  String get cancel => 'Скасувати';

  @override
  String get delete => 'Видалити';

  @override
  String get create_question => 'Створити питання';

  @override
  String get something_went_wrong => 'Щось пішло не так';

  @override
  String get delete_question => 'Видалити запитання';

  @override
  String get are_you_sure_you_want_to_delete_question =>
      'Ви впевнені, що хочете видалити це запитання?';

  @override
  String get discover_meaningful_questions =>
      'Відкрий для себе глибокі запитання, що надихають на змістовні та щирі розмови з TrueSoul Cards.';

  @override
  String get about => 'Про гру';

  @override
  String get about_game => 'Про гру';

  @override
  String get info_title => 'Пізнавайте, спілкуйтесь, роздумуйте';

  @override
  String get info_description_part1 =>
      'Ця карткова гра створена для того, щоб зробити ваші вечори особливими та наповненими щирим спілкуванням з друзями, рідними або новими знайомими.';

  @override
  String get info_description_part2 =>
      'Ці запитання допоможуть краще відчути інших — їхні цінності, емоції та досвід. А ще вони надихають заглянути в себе, поділитися своїми думками й бути відкритим до нового.';

  @override
  String get continuePreviousGame => 'Продовжити попередню гру?';

  @override
  String get continuePreviousGameDescription =>
      'Бажаєте продовжити попередню гру чи почати нову?';

  @override
  String get newGame => 'Нова гра';

  @override
  String get continueGame => 'Продовжити';

  @override
  String get processing_your_request => 'Обробка вашого запиту...';

  @override
  String get ads_removed_successfully => 'Рекламу успішно видалено!';

  @override
  String get error_removing_ads => 'Помилка під час видалення реклами';

  @override
  String get restoring_your_purchases => 'Відновлення ваших покупок...';

  @override
  String get purchases_restored_successfully => 'Покупки успішно відновлено!';

  @override
  String get error_restoring_purchases => 'Помилка під час відновлення покупок';

  @override
  String get play => 'Грати';

  @override
  String get upload_select_category_first => 'Спочатку оберіть категорію.';

  @override
  String get upload_failed_read_file => 'Не вдалося прочитати вибраний файл.';

  @override
  String get upload_json_error_root_must_be_array =>
      'Корінь має бути масивом JSON.';

  @override
  String get upload_json_error_item_must_be_object =>
      'Кожен елемент масиву має бути об\'єктом JSON.';

  @override
  String get upload_json_error_needs_language_key =>
      'Кожне запитання має містити принаймні один мовний ключ (непорожній рядок).';

  @override
  String get upload_failed_parse_json_file => 'Не вдалося розібрати JSON-файл.';

  @override
  String get upload_choose_json_first => 'Спочатку оберіть JSON-файл.';

  @override
  String get default_questions_json_filename => 'questions.json';

  @override
  String get upload_importing => 'Імпорт...';

  @override
  String get upload_status_failed => 'Не вдалося завантажити';

  @override
  String get upload_subtitle_secure_import =>
      'Iмпортуйте JSON-запитання до обраної категорії.';

  @override
  String get ai_prompt_text =>
      'Згенеруй 15 глибоких, емоційних запитань для саморефлексії та змістовної розмови.\n\nПоверни ТІЛЬКИ JSON-масив.\n\nКожен елемент має містити:\n- \"en\" (англійською)\n- \"uk\" (українською, природно й легко для читання)';

  @override
  String get upload_only_json_files => 'Підтримуються лише файли .json.';

  @override
  String get upload_failed_read_dropped_file =>
      'Не вдалося прочитати перетягнутий файл.';

  @override
  String get upload_drop_error_unknown => 'невідомо';

  @override
  String get upload_tap_or_drop_files =>
      'Торкніться, щоб обрати, або перетягніть файли';

  @override
  String get upload_json_format_hint =>
      'JSON-файл (масив об\'єктів із мовними ключами)';

  @override
  String get upload_paste_questions_title => 'Вставити запитання';

  @override
  String get upload_paste_questions_hint => 'Кожне запитання з нового рядка';

  @override
  String get upload_paste_questions_helper =>
      'Запитання буде додано мовою, обраною як основна в налаштуваннях.';

  @override
  String get upload_use_pasted_questions => 'Використати вставлені запитання';

  @override
  String get upload_plain_questions_empty => 'Вставте хоча б одне запитання.';

  @override
  String upload_plain_questions_ready(int count) {
    return '$count запитань готово до імпорту';
  }

  @override
  String get browse_json => 'Завантажити JSON';

  @override
  String get upload_preview_title => 'Перегляд';

  @override
  String get upload_section_import_in_progress => 'Триває імпорт';

  @override
  String get upload_section_recently_uploaded => 'Нещодавно завантажено';

  @override
  String get upload_questions_uploaded_singular => 'Завантажено 1 запитання';

  @override
  String upload_questions_uploaded_plural(int count) {
    return 'Завантажено $count запитань';
  }

  @override
  String category_title_with_subcategory(String title, String subcategory) {
    return '$title ($subcategory)';
  }

  @override
  String questions_detected_count(int count) {
    return 'Виявлено запитань: $count';
  }

  @override
  String detected_languages_label(String languages) {
    return 'Виявлені мови: $languages';
  }

  @override
  String questions_preview_item(int index, String snippet) {
    return '$index. $snippet';
  }

  @override
  String file_size_mb(String value) {
    return '$value МБ';
  }

  @override
  String file_size_kb(String value) {
    return '$value КБ';
  }

  @override
  String drop_error_with_detail(String detail) {
    return 'Помилка перетягування: $detail';
  }

  @override
  String upload_categories_load_error(String error) {
    return 'Щось пішло не так: $error';
  }

  @override
  String upload_failed_with_error(String error) {
    return 'Не вдалося завантажити: $error';
  }

  @override
  String invalid_json_with_message(String message) {
    return 'Некоректний JSON: $message';
  }
}
